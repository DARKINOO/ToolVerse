  ...
  return (
    ...
    <h1 className='font-bold text-center text-xl'>
      ...
      <div className="addTodo my-5 flex flex-col gap-4">
        ...
        <button onClick={handleAdd} disabled={todo.length<=3} className='bg-violet-700 p-2 py-2  text-white text-sm font-bold rounded-md  disabled:bg-violet-500 hover:bg-violet-950'>Save</button>
      </div>
        ...
        return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex md:w-1/2 my-3 justify-between">
          ...
          <input name={item.id} onChange={handleCheckbox} type="checkbox" checked={item.isCompleted} id="" />
            ...
            <div className="buttons flex h-full">
              <button onClick={(e)=>handleEdit(e, item.id)} className='bg-violet-700 p-2 py-1 text-white text-sm font-bold rounded-md mx-1 hover:bg-violet-950'><FaEdit/></button>
              <button onClick={(e)=>{handleDelete(e, item.id)}} className='bg-violet-700 p-2 py-1 text-white text-sm font-bold rounded-md mx-1 hover:bg-violet-950'><MdDelete/></button>
            </div>
          </div>
          })}
        </div>
